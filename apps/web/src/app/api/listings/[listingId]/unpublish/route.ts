import { unpublishListing } from "@/lib/api";
...
export async function POST(request: Request, context: { params: Promise<{ listingId: string }> }) {
  ...
  const result = await unpublishListing(listingId, sessionResult.session.accessToken);

  return redirectAfterMutation(request, returnTo, result.ok
    ? { status: "listing-unpublished" }
    : { error: "listing-unpublish-failed" });
}
